...
2. **I/O Activities**: Recording a frame to storage while video capturing is executed may cause delay in for frame grabbing as I/O time is highly varied. Although we use SSD and perform I/O in another thread separate from a frame grabbing thread, we found the I/O thread could still interfere with frame-grabbing timing, especially when your CPU is not capable (e.g., only two physical cores and low clock speed). If your video is relatively short (e.g. <= 5 minutes), you may set io_buffer_length to cover the entire set of the incoming frames. In this case, VidCap Pacer will not write any frame to storage until all frames are captured and kept in the buffer. This will minimize interference from I/O activities. Fortunately, many recent computers are equipped with a better SSD and more performant CPU. This issue may not be serious unless you want to write high resolution frames.
